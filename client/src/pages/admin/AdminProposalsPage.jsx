                ...
                {rows.map((p) => {
                  ...
                  return (
                    <tr key={p._id} className="text-slate-800">
                      ...
                      <td className="py-3 pr-4">{formatMoney(p.currentSalarySnapshot)}</td>
                      <td className="py-3 pr-4 font-medium">{formatMoney(p.proposedNewSalary)}</td>
                      <td className="py-3 pr-4">{formatMoney(p.costOfChange)}</td>
                      <td className="py-3 pr-4">
        ...
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/40 p-4 sm:items-center">
          <div className="w-full max-w-lg rounded-xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-slate-600">
              {decision.proposal.employee?.name} · {formatMoney(decision.proposal.currentSalarySnapshot)} →{' '}
              {formatMoney(decision.proposal.proposedNewSalary)}
            </p>
